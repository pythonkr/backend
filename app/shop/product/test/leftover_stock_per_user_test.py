...
@pytest.fixture
def group_with_options(product) -> tuple[OptionGroup, Option, Option]:
    """1 group with 2 options — 무제한 / 무한 재고 baseline."""
    group = OptionGroup.objects.create(product=product, name="사이즈")
...
def _serialize_via_api(client, product) -> dict:
    response = ProductsApi(http_client=client).retrieve(product.id)
    assert response.status_code == HTTP_200_OK
    return response.json()


def _add_to_cart(user, product, option, *, status=OrderProductRelation.OrderProductStatus.pending) -> None:
    order = Order.objects.create(user=user, name=f"o-{user.id}")
    opr = OrderProductRelation.objects.create(order=order, product=product, price=product.price, status=status)
...
def test_anonymous_user_sees_full_limits_only(anon_client, product, group_with_options):
    ...
    data = _serialize_via_api(anon_client, product)
    [grp] = data["option_groups"]
    assert grp["leftover_stock_per_user"] == 5  # group 한도가 binding (product / stock 무제한)
...
def test_breakdown_subtracts_cart_and_purchased_for_logged_in_user(
    customer_client, customer_user, product, group_with_options
):
    ...
    _add_to_cart(customer_user, product, a, status=OrderProductRelation.OrderProductStatus.pending)
    _add_to_cart(customer_user, product, a, status=OrderProductRelation.OrderProductStatus.paid)

    data = _serialize_via_api(customer_client, product)
    [grp] = data["option_groups"]
...
@pytest.mark.django_db
def test_product_level_limit_can_be_binding_constraint(customer_client, customer_user, product, group_with_options):
    # Product.max_quantity_per_user 가 group / option 한도 보다 작아서 binding 이 되는 경우 — 명시적 케이스.
    product.max_quantity_per_user = 2
    product.save()
    group, a, _b = group_with_options
    ...
    assert option_a["leftover_stock_per_user"] == 2  # product 한도가 binding


@pytest.mark.django_db
def test_option_leftover_stock_propagates_to_breakdown(customer_client, customer_user, product, group_with_options):
    group, a, _b = group_with_options
    a.stock = 4
    a.save()
...
def test_other_users_cart_does_not_affect_this_user(
    customer_client, customer_user, other_user, product, group_with_options
):
    ...
    _add_to_cart(other_user, product, a, status=OrderProductRelation.OrderProductStatus.pending)
    _add_to_cart(other_user, product, a, status=OrderProductRelation.OrderProductStatus.pending)
...
@pytest.mark.django_db
def test_taken_at_or_above_limit_returns_zero_not_negative(customer_client, customer_user, product, group_with_options):
...
@pytest.mark.django_db
def test_refunded_orders_do_not_count_toward_user_usage(customer_client, customer_user, product, group_with_options):
    # refunded 는 PURCHASED_STOCK_STATUS / pending 어디에도 속하지 않아 누적에서 제외돼야 한다.
    group, a, _b = group_with_options
    a.max_quantity_per_user = 3
    a.save()
    _add_to_cart(customer_user, product, a, status=OrderProductRelation.OrderProductStatus.refunded)
...
def test_list_query_count_is_independent_of_product_and_option_count(
    customer_client, customer_user, product, django_assert_max_num_queries
):
    ...
    all_products = [product, *extra_products]
...
@pytest.mark.django_db
def test_soft_deleted_options_are_excluded_from_response(customer_client, product, group_with_options):
...
@pytest.mark.django_db
def test_custom_response_opor_counts_group_quota_but_no_option(customer_client, customer_user, product):
    ...
    opr = OrderProductRelation.objects.create(
        order=order, product=product, price=product.price, status=OrderProductRelation.OrderProductStatus.paid
    )
    OrderProductOptionRelation.objects.create(
...
@pytest.mark.django_db
def test_single_product_cart_opr_is_excluded_from_counts(customer_client, customer_user, product, group_with_options):
...
@pytest.mark.django_db
def test_direct_dto_instantiation_falls_back_to_model_property(customer_user, product, group_with_options):
    ...
    data = ProductDto(instance=product).data
    option_a = next(o for o in data["option_groups"][0]["options"] if o["id"] == str(a.id))
...
@pytest.mark.django_db
def test_all_unlimited_returns_none(anon_client, product, group_with_options):
