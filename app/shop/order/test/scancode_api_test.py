...
@pytest.mark.django_db
def test_scancode_user_token_returns_user_orders(anon_client, completed_order, customer_user):
    response = ScanCodeApi(http_client=anon_client).list({"token": customer_user.scancode_token})
    assert response.status_code == HTTP_200_OK


@pytest.mark.django_db
def test_scancode_user_token_rejects_when_all_orders_refunded(anon_client, refunded_order, customer_user):
    response = ScanCodeApi(http_client=anon_client).list({"token": customer_user.scancode_token})
    assert response.status_code == HTTP_403_FORBIDDEN


@pytest.mark.django_db
def test_scancode_order_token_returns_order(anon_client, completed_order):
    response = ScanCodeApi(http_client=anon_client).list({"token": completed_order.scancode_token})
    assert response.status_code == HTTP_200_OK


@pytest.mark.django_db
def test_scancode_order_token_rejects_refunded_order(anon_client, refunded_order):
...
@pytest.mark.django_db
def test_scancode_opr_token_returns_order_product(anon_client, completed_order):
...
@pytest.mark.django_db
def test_scancode_opr_token_with_invalid_salt_rejects(anon_client, completed_order):
    opr = completed_order.products.first()
