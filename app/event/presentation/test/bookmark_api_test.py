...
def list_url(event_name: str | None = None) -> str:
    url = reverse("v1:presentation-bookmark-list")
    if event_name:
        url += f"?event={event_name}"
    return url


def detail_url(presentation_id: uuid.UUID | str) -> str:
    return reverse("v1:presentation-bookmark-detail", kwargs={"pk": str(presentation_id)})


# ══════════════════════════════════════════════
# GET /v1/event/presentation/bookmarks/
# ══════════════════════════════════════════════
...
class TestBookmarkList:
    ...
    def test_returns_empty_list_when_no_bookmarks(self, authed_client: APIClient, event: Event):
        ...
        """
        response = authed_client.get(list_url(event.name))
    ...
    def test_returns_bookmarked_presentation_ids(
        ...
        """
        PresentationBookmark.objects.create(user=user, event=event, presentation=presentation)
        PresentationBookmark.objects.create(user=user, event=event, presentation=presentation_2)
    ...
    @pytest.mark.django_db
    def test_filters_by_event_name(
        self,
        authed_client: APIClient,
        user: UserExt,
        event: Event,
        ...
        """
        event 쿼리 파라미터로 필터링했을 때, 해당 행사의 북마크만 반환되는지 검증합니다.
        다른 행사의 북마크가 섞여 나오면 안 됩니다.
        """
        PresentationBookmark.objects.create(user=user, event=event, presentation=presentation)
        PresentationBookmark.objects.create(user=user, event=other_event, presentation=other_event_presentation)
    ...
    @pytest.mark.django_db
    def test_defaults_to_latest_event_when_no_event_param(
        self, authed_client: APIClient, user: UserExt, event: Event, other_event: Event, presentation: Presentation
    ):
        """
        event 쿼리 파라미터를 생략하면, 기존 presentation API와 동일하게
        가장 최신 행사(event_start_at 기준)의 북마크를 반환하는지 검증합니다.
        """
        PresentationBookmark.objects.create(user=user, event=event, presentation=presentation)
        ...
        assert response.status_code == http.HTTPStatus.OK
        # event(2026)이 other_event(2025)보다 최신이므로 event의 북마크가 반환되어야 함
        assert str(presentation.id) in response.json()["presentation_ids"]
    ...
    def test_does_not_return_other_users_bookmarks(
        ...
        """
        PresentationBookmark.objects.create(user=other_user, event=event, presentation=presentation)
    ...
    def test_unauthenticated_returns_403(self, anon_client: APIClient, event: Event):
        ...
        """
        response = anon_client.get(list_url(event.name))
    ...
    def test_nonexistent_event_returns_404(self, authed_client: APIClient):
        """
        존재하지 않는 행사 이름으로 요청하면 404를 반환하는지 검증합니다.
        """
        response = authed_client.get(list_url("존재하지 않는 행사"))

        assert response.status_code == http.HTTPStatus.NOT_FOUND


# ══════════════════════════════════════════════
# POST /v1/event/presentation/bookmarks/
# ══════════════════════════════════════════════
...
class TestBookmarkCreate:
    ...
    def test_sets_event_from_presentation(
        ...
        """
        authed_client.post(list_url(), data={"presentation_id": str(presentation.id)}, format="json")

        bookmark = PresentationBookmark.objects.get(user=user, presentation=presentation)
        assert bookmark.event_id == event.id
    ...
    @pytest.mark.django_db
    def test_soft_deleted_presentation_returns_404(self, authed_client: APIClient, presentation: Presentation):
        """
    ...
    @pytest.mark.django_db
    def test_invalid_presentation_id_format_returns_400(self, authed_client: APIClient):
        """
    ...
    def test_missing_presentation_id_returns_400(self, authed_client: APIClient):
        """
        요청 바디에 presentation_id 필드가 없으면 400 Bad Request를 반환하는지 검증합니다.
        """
        response = authed_client.post(list_url(), data={}, format="json")

        assert response.status_code == http.HTTPStatus.BAD_REQUEST

    @pytest.mark.django_db
    def test_unauthenticated_returns_403(self, anon_client: APIClient, presentation: Presentation):
        """
    ...
    def test_allows_overlapping_time_sessions(
        self, authed_client: APIClient, user: UserExt, presentation: Presentation, presentation_2: Presentation
    ):
        """
        시간이 겹치는 세션들도 모두 북마크할 수 있는지 검증합니다.
        UX 확정 사항: 겹침 경고는 프론트가 처리하고, 서버는 시간대 충돌을 검사하지 않습니다.
        """
        resp1 = authed_client.post(list_url(), data={"presentation_id": str(presentation.id)}, format="json")
        resp2 = authed_client.post(list_url(), data={"presentation_id": str(presentation_2.id)}, format="json")
...
# ══════════════════════════════════════════════
# DELETE /v1/event/presentation/bookmarks/{presentation_id}/
# ══════════════════════════════════════════════
...
class TestBookmarkDestroy:
    ...
    def test_deletes_bookmark_and_returns_204(
        ...
        response = authed_client.delete(detail_url(presentation.id))
    ...
    def test_nonexistent_presentation_returns_404(self, authed_client: APIClient):
        ...
        """
        response = authed_client.delete(detail_url(uuid.uuid4()))
    ...
    def test_unauthenticated_returns_403(self, anon_client: APIClient, presentation: Presentation):
        ...
        IsAuthenticated 권한 체크에서 403을 반환합니다.
        """
        response = anon_client.delete(detail_url(presentation.id))
    ...
    def test_only_deletes_specified_bookmark(
        ...
        authed_client.delete(detail_url(presentation.id))
...
class TestBookmarkUndoFlow:
    ...
    def test_add_remove_re_add_flow(
        ...
        """
        # 1단계: 담기
        resp = authed_client.post(list_url(), data={"presentation_id": str(presentation.id)}, format="json")
        assert resp.status_code == http.HTTPStatus.CREATED

        # GET으로 확인: 1개
        resp = authed_client.get(list_url(event.name))
        assert len(resp.json()["presentation_ids"]) == 1

        # 2단계: 빼기
        resp = authed_client.delete(detail_url(presentation.id))
    ...
    def test_rapid_double_delete_is_safe(
        ...
        resp1 = authed_client.delete(detail_url(presentation.id))
        resp2 = authed_client.delete(detail_url(presentation.id))

        assert resp1.status_code == http.HTTPStatus.NO_CONTENT
        assert resp2.status_code == http.HTTPStatus.NO_CONTENT

    @pytest.mark.django_db
    def test_rapid_double_post_is_safe(self, authed_client: APIClient, user: UserExt, presentation: Presentation):
        """
        같은 세션에 대해 POST가 빠르게 2번 연속 호출되어도
        두 번째 요청이 에러 없이 200을 반환하고 중복 레코드가 생기지 않는지 검증합니다.
        """
        resp1 = authed_client.post(list_url(), data={"presentation_id": str(presentation.id)}, format="json")
        resp2 = authed_client.post(list_url(), data={"presentation_id": str(presentation.id)}, format="json")
...
class TestErrorResponseFormat:
    ...
    @pytest.mark.django_db
    def test_400_validation_error_follows_envelope(self, authed_client: APIClient):
        """
